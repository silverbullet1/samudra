...
#if 0 // Define this for simple stack overflow function.
...
void vApplicationStackOverflowHook( TaskHandle_t xTask, portCHAR *pcTaskName )
{
	...
	DDRB  |= _BV(DDB7);
	PORTB |= _BV(PORTB7);       // Main (red PB7) LED on. Main LED on and die.
	...
	DDRB  |= _BV(DDB5);
	PORTB |= _BV(PORTB5);       // Main (red PB5) LED on. Main LED on and die.
